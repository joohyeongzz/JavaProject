...
public class A03_variable {
	...
	public static void main(String[] args) {
		...
		//  - 문자, 밑줄, 달러기호로 시작할 수 있음. 숫자로 시작할 수 없음.
		//  - 식별자에는 공백을 포함할 수 없음. ex) is child(x) -> isChild, is_child
		//  - 키워드는 식별자로 사용하면 안됨.
		 ...
		 *  > 첫글자, 연결되는 단어의 첫글자는 대문자로 표기. 나머지는 소문자로 표기. ex) CustomerManager
		 *  > $는 내부 클래스에서 특별한 의미를 가지고 있기 떄문에 사용을 권장하지 않음.
		 *  
		 * - 변수 이름
		 *  > 명사적 의미를 갖게 만들어 준다.
		 *  > 첫글자는 소문자로, 연결되는 단어의 첫글자는 대문자로 표기, 나머지는 소문자로 표기. ex) customerName
		 *  > 일반적으로 변수 이름에는 "_"를 사용하지 않음.
		 *  
		 * - 메서드 이름
		 *  > 동사적 의미를 갖게 작성한다.
		 *  > 첫글자는 소문자로, 연결되는 단어의 첫글자는 대문자로 표기, 나머지는 소문자로 표기. ex) insertCustomerData()
		...
		int asa = 20; // 같은 이름의 변수는 밑에서 다시 선언 할 수 없음.
		a = 20;     // 재사용은 가능함.
		// ** int는 정수 타입을 나타내는 키워드
		
		
		 // 변수를 사용하는 이유
		 // 1. 값을 일괄적으로 변경할 수 있게 하기 위해서 (같은 값을 여러번 사용해야 하는 경우 편리함)
